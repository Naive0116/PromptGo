...
### v1.4 (2026-01-29)
- 🧠 **SocraticEngine Prompt v3.0**：智能输入识别（`input_spec`）+ 用户输入区块 + 推理策略/异常处理/初始化占位符
- 🧩 **四大思维原则**：苏格拉底诘问法 / 第一性原理 / 布鲁姆认知金字塔 / 水平思考法（用于追问与提示词生成引导）
- 🧷 **交互修复**：选项确认提交使用 `label` 展示与发送，避免显示内部 `value`
- 🧱 **设置弹窗 UI 修复**：卡片网格尺寸统一；滑块刻度文案按真实刻度对齐（值为 5 对齐“标准”）
- 🧾 **文档同步**：`prd.md` 数据契约与 2026-01-29 更新日志补充（Prompt v3.0 + UI 修复）
- 🚚 **仓库迁移提示**：GitHub 远端提示仓库已迁移到 `https://github.com/Naive0116/PromptGo.git`
